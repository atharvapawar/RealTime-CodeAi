...
class DocDiffer {
  ...
  async processDocs(sourceTypes = []) {
    try {
      ...
      const sourcesToProcess = sourceTypes.length > 0 
        ? this.docSources.filter(source => sourceTypes.includes(source.type))
        : this.docSources;
      
      if (sourcesToProcess.length === 0) {
        return { success: false, error: 'No valid documentation sources specified' };
      }
      ...
      vscode.window.withProgress({
        ...
        for (let i = 0; i < sourcesToProcess.length; i++) {
          progress.report({ 
            message: `Fetching ${sourcesToProcess[i].name} (${i + 1}/${sourcesToProcess.length})`,
            increment: step 
          });
      ...
      const fetchPromises = sourcesToProcess.map(source => this.fetchLatestDocs(source));
