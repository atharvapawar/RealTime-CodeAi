...
function activate(context) {
  ...
  const webviewPanel = new WebviewPanel(context, contextExtractor, docDiffer);
  
  // Initialize collection status
  contextExtractor.getCollectionStatus().then(status => {
    console.log(`ChromaDB collection status: ${status.status}, documents: ${status.count}`);
    if (status.count === 0) {
      ...
      if (Object.keys(docsForChroma).length > 0) {
        contextExtractor.updateCollection(docsForChroma)
          .then(result => console.log('Initialized ChromaDB collection:', result))
          .catch(err => console.error('Failed to initialize ChromaDB collection:', err));
      }
    }
  });
    ...
    errorHandler.createErrorBoundary(async () => {
      ...
      const result = await vscode.window.withProgress({
        ...
        try {
          ...
          progress.report({ increment: 0, message: 'Starting documentation refresh...' });
          
          // Fetch and process the latest docs
          const processResult = await docDiffer.processDocs();
          
          if (!processResult.success) {
            throw new Error(processResult.error || 'Failed to process documentation');
          }
          
          progress.report({ increment: 70, message: 'Updating vector database...' });
          
          // Update the ChromaDB collection with the latest docs
          const docsForChroma = docDiffer.getDocsForChroma();
          const updateResult = await contextExtractor.updateCollection(docsForChroma);
          
          if (!updateResult.success) {
            throw new Error(updateResult.error || 'Failed to update vector database');
          }
  ...
  let getContextualHelp = vscode.commands.registerCommand(
    ...
    errorHandler.createErrorBoundary(async () => {
      ...
      vscode.window.withProgress({
        ...
        progress.report({ increment: 0, message: 'Analyzing code context...' });
        ...
        try {
          ...
          switch (document.languageId) {
            ...
            case 'typescript':
              ...
              } else if (linePrefix.includes('const') || linePrefix.includes('let')) {
  ...
  let searchDocs = vscode.commands.registerCommand(
    ...
    errorHandler.createErrorBoundary(async () => {
      ...
      const query = await vscode.window.showInputBox({
        ...
        prompt: 'Enter keywords to search in Next.js and Tailwind documentation'
      });
      ...
      // Find relevant docs
      const relevantDocs = await contextExtractor.findRelevantDocs({ surroundingCode: query }, 5);
